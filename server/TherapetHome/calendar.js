function CalendarControl() {
  ...
  const calendarControl = {
    ...
    selectDate: function (e) {
      ...
      const month = ('0' + (calendar.getMonth() + 1)).slice(-2);
      const day = ('0' + e.target.textContent).slice(-2);
      ...
      if (searchDate) {
          ...
          // Retrieve emotion data
          getEmotion(userID, searchDate)
              .then((doc) => {
                  if (doc.exists) {
                      var emotionData = doc.data().emotion;
                      document.getElementById("searchResult").innerText = `Emotion on ${searchDate}: ${emotionData}`;
                  } else {
                      document.getElementById("searchResult").innerText = `No emotion data found for ${searchDate}`;
                  }
              })
              .catch((error) => {
...
function getEmotion(userID, date) {
    var emotionsRef = db.collection("users").doc(userID).collection("emotions").doc(date);
    return emotionsRef.get();
}

function getDiaryEntry(userID, date) {
    var diaryRef = db.collection("users").doc(userID).collection("diary").doc(date);
    return diaryRef.get();
}